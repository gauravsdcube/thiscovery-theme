...
<div id="tc-custom-css-empty" class="text-body-secondary mb-2<?= empty($rules) ? '' : ' d-none' ?>">
    <?= Yii::t('ThiscoveryThemeModule.base', 'No custom CSS rules yet.') ?>
</div>

<div class="list-group mb-2" id="tc-custom-css-rules">
    <?php foreach ($rules as $rule): ?>
        ...
        ?>
        <div class="tc-custom-rule-item list-group-item">
            <div class="tc-custom-rule-summary d-flex justify-content-between align-items-center gap-2">
                <div class="tc-custom-rule-labels min-w-0">
                    <div class="tc-custom-rule-title text-truncate"><?= Html::encode($title) ?></div>
                    <div class="tc-custom-rule-subtitle text-body-secondary small text-truncate<?= ($description !== '' && $selector !== '') ? '' : ' d-none' ?>">
                        <?= Html::encode($selector) ?>
                    </div>
                </div>
                <div class="d-flex gap-1 flex-shrink-0">
                    <button type="button" class="btn btn-sm btn-secondary tc-edit-custom-rule"><?= Yii::t('ThiscoveryThemeModule.base', 'Edit') ?></button>
                    <button type="button" class="btn btn-sm btn-danger tc-delete-custom-rule"><?= Yii::t('ThiscoveryThemeModule.base', 'Delete') ?></button>
                </div>
            </div>
            <div class="tc-custom-rule-editor d-none mt-3 border-top pt-3">
                <div class="mb-2">
                    <label class="form-label"><?= Yii::t('ThiscoveryThemeModule.base', 'Description') ?></label>
                    <input type="text" class="form-control tc-custom-description" name="<?= Html::encode($customCssDescriptionName) ?>" value="<?= Html::encode($description) ?>" placeholder="<?= Html::encode(Yii::t('ThiscoveryThemeModule.base', 'e.g. Sidebar link hover colour')) ?>">
                </div>
                <div class="mb-2">
                    <label class="form-label"><?= Yii::t('ThiscoveryThemeModule.base', 'Item selector') ?></label>
                    <input type="text" class="form-control tc-custom-selector" name="<?= Html::encode($customCssSelectorName) ?>" value="<?= Html::encode($selector) ?>" placeholder=".list-group-item:hover">
                </div>
                <div class="mb-2">
                    <label class="form-label"><?= Yii::t('ThiscoveryThemeModule.base', 'CSS declarations') ?></label>
                    <textarea class="form-control tc-custom-declarations" name="<?= Html::encode($customCssDeclarationsName) ?>" rows="4" placeholder="background: #f3f2f1;"><?= Html::encode($declarations) ?></textarea>
                </div>
                <button type="button" class="btn btn-light btn-sm tc-cancel-custom-rule"><?= Yii::t('ThiscoveryThemeModule.base', 'Done') ?></button>
            </div>
        </div>
    <?php endforeach; ?>
</div>

<button type="button" class="btn btn-secondary btn-sm" id="tc-add-custom-rule"><?= Yii::t('ThiscoveryThemeModule.base', 'Add CSS item') ?></button>

<script <?= Html::nonce() ?>>
    (function () {
        function initGovukCustomCssRules() {
            const container = document.getElementById('tc-custom-css-rules');
            const emptyState = document.getElementById('tc-custom-css-empty');
            const addBtn = document.getElementById('tc-add-custom-rule');
            if (!container || !addBtn || addBtn.dataset.tcCustomCssInit === '1') {
                return;
            }

            addBtn.dataset.tcCustomCssInit = '1';
            ...
            function updateEmptyState() {
                ...
                emptyState.classList.toggle('d-none', container.querySelectorAll('.tc-custom-rule-item').length > 0);
            }

            function closeEditor(item) {
                const editor = item.querySelector('.tc-custom-rule-editor');
                const editBtn = item.querySelector('.tc-edit-custom-rule');
                if (editor) {
            ...
            function closeAllEditors(exceptItem) {
                container.querySelectorAll('.tc-custom-rule-item').forEach(function (item) {
                    if (item !== exceptItem) {
                        closeEditor(item);
                    }
                });
            ...
            function syncRuleSummary(item) {
                const descriptionInput = item.querySelector('.tc-custom-description');
                const selectorInput = item.querySelector('.tc-custom-selector');
                const title = item.querySelector('.tc-custom-rule-title');
                const subtitle = item.querySelector('.tc-custom-rule-subtitle');
            ...
            function buildRuleNode(description, selector, declarations, openEditorOnCreate) {
                ...
                item.className = 'tc-custom-rule-item list-group-item';

                const summary = document.createElement('div');
                summary.className = 'tc-custom-rule-summary d-flex justify-content-between align-items-center gap-2';

                const labels = document.createElement('div');
                labels.className = 'tc-custom-rule-labels min-w-0';

                const title = document.createElement('div');
                title.className = 'tc-custom-rule-title text-truncate';
                title.textContent = getRuleTitle(description, selector);

                const subtitle = document.createElement('div');
                subtitle.className = 'tc-custom-rule-subtitle text-body-secondary small text-truncate';
                const showSubtitle = (description || '').trim() !== '' && (selector || '').trim() !== '';
                subtitle.textContent = (selector || '').trim();
                if (!showSubtitle) {
                ...
                editBtn.type = 'button';
                editBtn.className = 'btn btn-sm btn-secondary tc-edit-custom-rule';
                editBtn.textContent = editLabel;

                const deleteBtn = document.createElement('button');
                deleteBtn.type = 'button';
                deleteBtn.className = 'btn btn-sm btn-danger tc-delete-custom-rule';
                ...
                editor.className = 'tc-custom-rule-editor mt-3 border-top pt-3' + (openEditorOnCreate ? '' : ' d-none');
                ...
                descriptionInput.className = 'form-control tc-custom-description';
                ...
                selectorInput.className = 'form-control tc-custom-selector';
                ...
                declarationsInput.className = 'form-control tc-custom-declarations';
                ...
                doneBtn.type = 'button';
                doneBtn.className = 'btn btn-light btn-sm tc-cancel-custom-rule';
            ...
            container.addEventListener('click', function (event) {
                const editBtn = event.target.closest('.tc-edit-custom-rule');
                if (editBtn) {
                    event.preventDefault();
                    const item = editBtn.closest('.tc-custom-rule-item');
                    const editor = item?.querySelector('.tc-custom-rule-editor');
                    if (!item || !editor) {
                        return;
                    }
                ...
                const cancelBtn = event.target.closest('.tc-cancel-custom-rule');
                if (cancelBtn) {
                    event.preventDefault();
                    const item = cancelBtn.closest('.tc-custom-rule-item');
                    if (!item) {
                        return;
                    }
                    closeEditor(item);
                ...
                const deleteBtn = event.target.closest('.tc-delete-custom-rule');
                if (deleteBtn) {
                    event.preventDefault();
                    deleteBtn.closest('.tc-custom-rule-item')?.remove();
                    updateEmptyState();
                }
            });
            ...
            container.addEventListener('input', function (event) {
                if (
                    event.target.classList.contains('tc-custom-selector')
                    || event.target.classList.contains('tc-custom-description')
                ) {
                    syncRuleSummary(event.target.closest('.tc-custom-rule-item'));
                }
            });
